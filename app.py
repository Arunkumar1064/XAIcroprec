...
with col_m2:
    st.metric("Accuracy", f"{scores[selected_model_name]:.1%}")
...
if 'prediction_result' in st.session_state:
    ...
    with st.expander("Show Feature Importance (Global Summary)"):
        st.markdown("This plot shows the most important features across many samples.")
        if st.checkbox("Generate Summary Plot (May take a few seconds)"):
            # Use 50 samples for speed
            X_subset = X_test.iloc[:50]
            explainer = shap.TreeExplainer(model)
            shap_values_global = explainer.shap_values(X_subset)
            
            # Handle class names for plotting
            class_names = list(le.classes_)
            
            # Explicitly create figure to avoid Streamlit warning
            fig = plt.figure()
            shap.summary_plot(shap_values_global, X_subset, class_names=class_names, show=False)
            st.pyplot(fig, bbox_inches='tight')
            plt.close(fig)
